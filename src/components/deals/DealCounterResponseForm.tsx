...
interface DealCounterResponseFormProps {
  refetch?: () => any;
  dealId: string;
  businessName: string;
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      const actionMessages: Record<ActionType, string> = {
        COUNTER_PROPOSAL_ACCEPTED:
          "Counter Proposal Accepted and rate locked for the business",
        COUNTER_PROPOSAL_DECLINED: "Deal Proposal rejected",
  ...
  return (
    <>
      ...
      <ConfirmationDialog
        ...
        title={`Confirm ${action === "COUNTER_PROPOSAL_ACCEPTED" ? "Accepted" : action === "COUNTER_PROPOSAL_DECLINED" ? "Rejection" : "Counter Proposal"}`}
        description={
          action === "COUNTER_PROPOSAL_ACCEPTED"
            ? `Accepted the requested rate of ${requestedRate} ${currency} for ${businessName}? This will lock the rate for the deal validity period.`
        ...
        variant={
          action === "COUNTER_PROPOSAL_DECLINED" ? "destructive" : "default"
        }
      />
    </>
  );
...
export default DealCounterResponseForm;
