...
const Login: React.FC = () => {
  ...
  const [, setCookie] = useCookies(["token", "refreshToken", "role", "tempToken", "twoFactorMethod"]);
  ...
  const onSubmit = async (values: LoginFormData, { setSubmitting }: any) => {
    ...
    try {
      ...
      const { requiresTwoFactor, tempToken, twoFactorMethod, accessToken: responseAccessToken, refreshToken, expiresIn, token } = apiData.data;
