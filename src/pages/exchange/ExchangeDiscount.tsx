...
//         try {
//             await axios.patch(
//                 `${BASE_URL}/api/v1/discount/${editingDiscountId}`,
//                 {
//                     name: form.name,
//                     description: form.description,
//                     type: form.type,
...
//         };
//         return map[status as keyof typeof map] || { variant: "outline", label: status, icon: Clock };
//     };
...
//                             </CardHeader>
//                             <CardContent>
//                                 <div className="text-2xl font-bold text-orange-600">{stats.inactiveCount}</div>
//                             </CardContent>
//                         </Card>
...
//                                                             Discount Code: <span className="font-mono">{discount.discountCode || "—"}</span>
//                                                         </p>
//                                                         <p className="text-sm text-muted-foreground mt-0.5">
//                                                             Discount Type: <span className="font-mono">{discount.type?.replace(/_/g, " ") || "—"}</span>
//                                                         </p>
//                                                     </div>
//                                                 </div>
//                                             </div>
...
//                                                     <span>{getTypeLabel(discount.type, discount.discountValue)}</span>
//                                                 </div>
//                                                 <div className="flex items-center gap-1.5">
...
const ExchangeDiscount = () => {
    ...
    const createDiscount = async () => {
        ...
        const newDiscount: Discount = {
            id: tempId,
            name: form.name,
            discountCode: "NEW", // temporary - will be replaced after real fetch
            description: form.description,
            type: form.type,
            discountValue: parseFloat(form.discountValue),
            status: form.status,
        ...
        setStats((prev) =>
            prev
                ? {
                      ...
                      [`${form.status.toLowerCase()}Count`]:
                          (prev as any)[`${form.status.toLowerCase()}Count`] + 1,
                      [form.type === "PERCENTAGE" ? "percentageCount" : "fixedAmountCount"]:
                          (prev as any)[form.type === "PERCENTAGE" ? "percentageCount" : "fixedAmountCount"] + 1,
                  }
                : null
        );
        ...
        try {
            ...
            // Refetch real data after success (will replace fake item)
            setTimeout(() => {
                setPage(0);
                fetchDiscounts();
            }, 800);
    ...
    return (
        <ExchangeLayout>
            <div className="space-y-8">
                ...
                {/* Stats + Search + List + Pagination remain the same as before */}
                {/* Stats Overview */}
                {stats && (
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                        <Card>
