...
const AdminKybMapping = () => {
  ...
  // ── Pagination state (server-side, Spring 0-indexed) ──
  ...
  // ── Auth headers — built fresh each call so token is always current ──
  const getHeaders = () => ({
    Authorization: `Bearer ${token}`,
  });

  // ── Fetch: Mappings (server-side paginated) ──
  const fetchMappings = async (page = currentPage) => {
    if (!token) return;
  ...
  // ── Fetch: Dropdowns for the mapping dialog ──
  // Called once on mount (when token is ready) and after creating new BT/KYB types
  const fetchDropdowns = async () => {
    ...
    try {
      const [btRes, kybRes] = await Promise.all([
        ...
        axios.get(
          `${BASE_URL}/api/v3/admin/kyb/master/kyb-types`,
          { headers: getHeaders() }
        ),
      ]);
      // Both APIs return: { status, message, statusCode, data: [...] }
  ...
  // Re-fetch when page changes (only after initial load)
  useEffect(() => {
    if (token) {
  ...
  const handleCreateBT = async () => {
    ...
    try {
      ...
      // Refresh dropdowns so new BT appears in mapping dialog
      fetchDropdowns();
    } catch (err: any) {
      toast({
        title: "Error",
  ...
  const handleCreateKYB = async () => {
    ...
    try {
      ...
      // Refresh dropdowns so new KYB type appears in mapping dialog
      fetchDropdowns();
    } catch (err: any) {
      toast({
        title: "Error",
  ...
  // Sends: { businessTypeId (number), kybTypeId (number), active, isDefault, description }
  ...
  // ── Open mapping dialog: re-fetch dropdowns to ensure latest data ──
  ...
  const highRiskCount = mappings.filter((m) => m.kybTypeCode === "HIGH_RISK").length;
  const defaultCount = mappings.filter((m) => m.default).length;

  // ── Client-side search within current page ──
  const filtered = mappings.filter(
    (m) =>
      m.businessType.toLowerCase().includes(search.toLowerCase()) ||
      m.kybTypeName.toLowerCase().includes(search.toLowerCase()) ||
      m.businessTypeCode.toLowerCase().includes(search.toLowerCase()) ||
      m.kybTypeCode.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <AdminLayout>
      ...
      {/* ══════════════════════════════════════════════════════════
          ...
          Dropdowns show: name (display) / id (value passed to API)
      ══════════════════════════════════════════════════════════ */}
      <Dialog open={showCreateMapping} onOpenChange={setShowCreateMapping}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            {/* Business Type dropdown — shows name, passes id */}
            <div className="space-y-1">
              <Label>
                ...
                <SelectContent>
                  {businessTypes.length === 0 && !loadingDropdowns ? (
                    ...
                    businessTypes.map((bt) => (
                      // value = String(bt.id) → converted to Number before sending
            ...
            {/* KYB Type dropdown — shows name, passes id */}
            <div className="space-y-1">
              <Label>
              ...
              >
                ...
                <SelectContent>
                  {kybTypes.length === 0 && !loadingDropdowns ? (
                    ...
                    kybTypes.map((kt) => (
                      // value = String(kt.id) → converted to Number before sending
...
export default AdminKybMapping;