...
const UserProfile = () => {
  ...
  const loadDocumentsFromServer = async () => {
    ...
    try {
      const res = await axios.get(`${BASE_URL}/api/business/${id}/kyb-context`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const transformed: Document[] = uploadedDocs.map((doc: KYBContextDocument) => {
        const docData = doc.document!;

        let uploadDate = new Date().toISOString().split("T")[0];
        if (docData.uploadedAt && Array.isArray(docData.uploadedAt) && docData.uploadedAt.length >= 3) {
        ...
        return {
          ...
          status: doc.verified === true ? "verified" : doc.verified === false ? "rejected" : "pending_review",
          documentId: docData.id,
          fileUrl: docData.fileUrl,
          documentNumber: docData.documentNumber,
          verified: doc.verified,
    ...
    const fetchKYBContext = async () => {
      try {
        ...
        if (data?.documents) {
          ...
          const types = data.documents.map((doc: KYBContextDocument) => doc.name);
          setDocumentTypes(types);
  ...
  const uploadDocument = async () => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        // IMPORTANT: Reload real data from server
        await loadDocumentsFromServer();
